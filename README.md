...
Предположим,что данный demo-сервис — это часть финтех-приложение (специализированное приложение для инвестиций), которое предоставляет клиентам (физическим лицам) возможность покупать и продавать ценные бумаги (акции, облигации, ETF). 
...
└── wiremock/
    ...
    └── mappings/
        ├── create-order.json
        ├── create-order-busy.json
        ├── create-order-fail.json
        ├── create-order-limit.json
        ├── get-order-filled.json
        ├── get-order-partial.json
        ├── get-order-pending.json
        └── oauth-token.json
```
...
Разберём путь данных на примере создания заявки на покупку ценных бумаг: от HTTP-запроса клиента до асинхронной отправки брокеру, обработки ответа и фонового опроса статуса исполнения. В основе — паттерн **Transactional Outbox** и **RabbitMQ**.
...
`Idempotency-Key` генерирует фронтенд (защита от двойной покупки при ретрае сети).
...
1. **Запрос от клиента (Фронтенд -> API):**
  - Клиент отправляет `POST /api/v1/investments/orders` → [`CreateOrderAction`](main-services/integration-broker-api/app/src/Infrastructure/Http/Action/Api/CreateOrderAction.php).
...
2. **Открытие транзакции:**
  - [`CreateOrderHandler`](main-services/integration-broker-api/app/src/Application/Command/CreateOrder/CreateOrderHandler.php) открывает транзакцию в PostgreSQL.
...
1. **Фоновое чтение (Relay / Publisher Worker):**
  - [`RelaySendOrderToBrokerHandler`](main-services/integration-broker-api/app/src/Infrastructure/Messenger/Handler/RelaySendOrderToBrokerHandler.php) непрерывно опрашивает таблицу `messenger_messages` на наличие новых, необработанных записей.
...
1. **Чтение из очереди (Consumer Worker):**
  - [`ConsumeSendOrderToBrokerHandler`](main-services/integration-broker-api/app/src/Infrastructure/Messenger/Handler/ConsumeSendOrderToBrokerHandler.php) вычитывает сообщение `SendOrderToBrokerMessage` из очереди RabbitMQ и передаёт в [`SendOrderToBrokerHandler`](main-services/integration-broker-api/app/src/Application/Command/SendOrderToBroker/SendOrderToBrokerHandler.php).
...
1. **Поиск активных заявок:** [`OrderPollingScheduleProvider`](main-services/integration-broker-api/app/src/Infrastructure/Scheduler/OrderPollingScheduleProvider.php) с заданной периодичностью (например, раз в 5-10 секунд) публикует `PollOrderStatusesMessage`; [`PollOrderStatusesHandler`](main-services/integration-broker-api/app/src/Application/Command/PollOrderStatuses/PollOrderStatusesHandler.php) делает выборку из таблицы `orders` всех заявок, находящихся в нетерминальных статусах (например, `SENT_TO_BROKER`, `PARTIALLY_FILLED`).
...
1. **Запрос статуса:** Фронтенд периодически вызывает роут `GET /api/v1/investments/orders/{id}` → [`GetOrderAction`](main-services/integration-broker-api/app/src/Infrastructure/Http/Action/Api/GetOrderAction.php).
2. **Чтение из БД:** [`GetOrderHandler`](main-services/integration-broker-api/app/src/Application/Query/GetOrder/GetOrderHandler.php) **не делает** запросов к брокеру. Он выполняет быстрый `SELECT` из таблицы `orders` по `id`.
...
Для ручных HTTP-запросов: `main-services/integration-broker-api/app/request.http` (выбрать environment `dev` в PhpStorm). Перед первым запросом:
